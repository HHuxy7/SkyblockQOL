...
class LeapNameCommand : ClientCommandBase("hl") {
    ...
    override fun processCommand(sender: ICommandSender, args: Array<String>) {
        ...
        if (foundPlayer) {
            TextUtils.info("§6Selected Leap Highlight for username: §f${skyblockqolConfig.highlightLeapName}§6.")
        }
    }
    private fun findUserNameFor(input: String, isClassName: Boolean): Boolean {
        ...
        if (isClassName) {
            for (l in scoreboardList) {
                if (l.contains(input)) {
                    val strArr = l.split(" ")
                    skyblockqolConfig.highlightLeapName = strArr[1]
                    return true
                }
            }
            ...
            for (l in scoreboardList) {
                ...
                if (username.lowercase() == input.lowercase()) {
                    skyblockqolConfig.highlightLeapName = username
                    return true
                }
            }
