...
import skyblockqol.utils.ContainerNameUtil
...
object DungeonLeap {
	...
	fun onTick(event: TickEvent.ClientTickEvent) {
		...
		if (!skyblockqolConfig.highlightLeap) return
		...
		if (inLeapMenu && System.currentTimeMillis() - lastGuiOpenEvent < 300) {
			for (i in 11..15) {
				...
				if (Utils.stripColorCodes(stack.displayName).equals(skyblockqolConfig.highlightLeapName, true)) {
