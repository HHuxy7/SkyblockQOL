...
object DragonFeatures {
	...
	fun handleNewParticle(pID: Int, x: Double, y: Double, z: Double) {
		if (!skyblockqolConfig.dragonTimer) return
	...
	fun updateDragonDead() {
		if (!(skyblockqolConfig.dragonKillBox || skyblockqolConfig.lbWaypoints || skyblockqolConfig.decoyWaypoints || skyblockqolConfig.gyroWaypoints)) return
		if (!ScoreBoardUtils.isInM7) return
	...
	private fun renderDragonBoxes() {
		if (!skyblockqolConfig.dragonKillBox) return
		if (!ScoreBoardUtils.isInM7) return
		if (mc.thePlayer.positionVector.yCoord > 45) return
		// Blue
		if (isAlive("blue"))
	...
	fun renderP5Waypoints(event: RenderWorldLastEvent) {
		if (!(skyblockqolConfig.gyroWaypoints || skyblockqolConfig.lbWaypoints || skyblockqolConfig.decoyWaypoints)) return
		if (!ScoreBoardUtils.isInM7) return
		val playerVec = mc.thePlayer.positionVector
		if (playerVec.yCoord > 45) return
		if (skyblockqolConfig.gyroWaypoints) {
			val color = "§6"
			for (g in gyros) {
		...
		if (skyblockqolConfig.lbWaypoints) {
			val color = "§a"
			for (s in shoot) {
		...
		if (skyblockqolConfig.decoyWaypoints) {
			val color = "§3"
			for (d in decoys) {
